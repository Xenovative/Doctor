    ...
    fetch(`/admin/api/user-reports/${encodeURIComponent(userIp)}`)
        ...
        .then(data => {
            ...
            if (data.reports && data.reports.length > 0) {
                        ...
                        <div class="card-body">
                            ...
                            ${report.analysis_report ? `
                                ...
                                <div class="bg-light p-3 rounded">
                                    <pre style="white-space: pre-wrap; font-family: inherit;">${report.analysis_report}</pre>
                                </div>
                            ` : ''}
                        </div>
                    </div>
                ...
                reportsHtml = '<div class="alert alert-info">暫無分析報告記錄</div>';
            }
            
            document.getElementById('userReportsContent').innerHTML = `
                <div class="mb-3">
                    <h6>用戶 ${userIp} 的分析報告 (共 ${data.reports ? data.reports.length : 0} 筆)</h6>
                </div>
                ${reportsHtml}
            `;
        })
